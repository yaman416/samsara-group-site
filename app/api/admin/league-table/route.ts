...
export async function GET(req: NextRequest) {
  ...
  const ensure = (sid: string, cid: string) => {
    const key = `${sid}:${cid}`;
    if (!table[key]) {
      const c = clubMap[cid] ?? { name: "Unknown", short_code: "", logo_url: null, home_color: null };
      table[key] = { season_id: sid, club_id: cid, club_name: c.name, short_code: c.short_code, logo_url: c.logo_url, home_color: c.home_color, played: 0, won: 0, drawn: 0, lost: 0, goals_for: 0, goals_against: 0, goal_diff: 0, points: 0 };
    }
    return table[key];
  };
  ...
  return NextResponse.json(rows.map((r, i) => ({ ...r, position: i + 1 })));
}
